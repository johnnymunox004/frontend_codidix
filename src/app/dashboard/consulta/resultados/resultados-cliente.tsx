          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="min-w-full">
              ...
              <div className="bg-white divide-y divide-gray-200">
                {procesosProcesados().map((proceso) => {
                  const partes = extraerPartes(proceso.sujetosProcesales);
                  
                  return (
                    <div key={proceso.idProceso} className="grid grid-cols-12 text-sm p-4 hover:bg-gray-50">
                      <div className="col-span-2">
                        <div className="font-medium text-gray-800">{proceso.llaveProceso}</div>
                        <div className="text-xs text-gray-500 mt-1">{extraerTipoProceso(proceso.sujetosProcesales)}</div>
                      </div>
                      <div className="col-span-3">
                        <div className="text-gray-800">{proceso.despacho}</div>
                      </div>
                      <div className="col-span-2">
                        {partes.demandante && (
                          <div className="text-xs">
                            <span className="text-gray-500">Demandante:</span> 
                            <span className="text-gray-800 ml-1">{partes.demandante.length > 20 ? `${partes.demandante.substring(0, 20)}...` : partes.demandante}</span>
                          </div>
                        )}
                        {partes.demandado && (
                          <div className="text-xs mt-1">
                            <span className="text-gray-500">Demandado:</span> 
                            <span className="text-gray-800 ml-1">{partes.demandado.length > 20 ? `${partes.demandado.substring(0, 20)}...` : partes.demandado}</span>
                          </div>
                        )}
                      </div>
                      <div className="col-span-2">{formatearFecha(proceso.fechaUltimaActuacion)}</div>
                      <div className="col-span-1">{proceso.departamento}</div>
              ...
              {/* Paginación */}
              {resultados.paginacion.cantidadPaginas > 1 && (
                <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                  <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                      <p className="text-sm text-gray-700">
                        Mostrando <span className="font-medium">{(pagina - 1) * resultados.paginacion.registrosPagina + 1}</span> a <span className="font-medium">
                          {Math.min(pagina * resultados.paginacion.registrosPagina, resultados.paginacion.cantidadRegistros)}
                        </span> de <span className="font-medium">{resultados.paginacion.cantidadRegistros}</span> resultados
                      </p>
                    </div>
